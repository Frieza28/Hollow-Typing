...
namespace GIGA.PixelCableRenderer
{
    ...
    public class CableRenderer : MonoBehaviour
    {
		...
		public enum RenderingEnviromnent
		{
			[System.ComponentModel.Description("2D")]
			TwoD,
			[System.ComponentModel.Description("3D")]
			ThreeD
		}
		...
		public RenderingEnviromnent renderingEnvironment = RenderingEnviromnent.TwoD;
		public bool enableBillboard;
#if UNITY_EDITOR
		public bool forceDefaultMovementHandles;
#endif
		...
		private bool billboard_firstRun;
		protected Vector3 billboard_lastCameraPos = Vector3.zero;
		protected Quaternion billboard_lastCameraRot = Quaternion.identity;
        ...
        public virtual void Init()
		{
            ...
            if (spriteRenderer != null)
            {
				...
				this.spriteRenderer.sharedMaterial = this.defaultCableMaterial;
#if UNITY_EDITOR
				if (this.spriteRenderer.sharedMaterial == null || !this.spriteRenderer.sharedMaterial.name.StartsWith(this.defaultCableMaterial.name))
					this.spriteRenderer.sharedMaterial = this.defaultCableMaterial;
#endif
            ...
            if (cableMaterial != null && cableMaterial.GetFloat("_InverseCanvasSize") != 1 / (float)CANVAS_SIZE)
        ...
        public virtual void ApplyShaderProperties()
        {
            ...
            if (this.colorMode == ColorMode.IndexedColor && !indexedColorsSet && this.cableMaterial != null)
			...
			if (this.cableMaterial != null && (this.cableMaterial.IsKeywordEnabled("TEXTURE_ON") && !this.IsUsingTexture() || this.cableMaterial.IsKeywordEnabled("TEXTURE_OFF") && this.IsUsingTexture())) this.SetShaderTextureVariant(this.IsUsingTexture());
#endif
		...
		protected void ExecuteShaderTextureVariant(bool on, ref Material sharedStaticMaterial, ref Dictionary<int, Material> canvasMaterialsDictionary)
		{
			if (on)
			{
				...
				{
					if (this.cableMaterial == null)
					{
						StartCoroutine(DelayedExecuteShaderTextureVariant(on, sharedStaticMaterial, canvasMaterialsDictionary));
						return;
					}
		...
		private IEnumerator DelayedExecuteShaderTextureVariant(bool on, Material sharedStaticMaterial, Dictionary<int, Material> canvasMaterialsDictionary)
		{
			// When instantiating cables in the world for the first time, without having any in scene it may happen that the shared material
			// was null, waiting until it has been found and then retry
			if (this.spriteRenderer != null)
			{
				do
				{
				...
				while (this.cableMaterial == null);

				if (this.cableMaterial != null)
				{
					this.ExecuteShaderTextureVariant(on, ref sharedStaticMaterial, ref canvasMaterialsDictionary);
					// Setting general shader properties
					if (cableMaterial != null && cableMaterial.GetFloat("_InverseCanvasSize") != 1 / (float)CANVAS_SIZE)
						cableMaterial.SetFloat("_InverseCanvasSize", 1 / (float)CANVAS_SIZE);
					this.ApplyShaderProperties();
		...
		/// <summary>
		/// Return the left anchor point of the cable
		/// </summary>
		/// <returns></returns>
		public Vector3 GetLeftPoint()
		{
			float halfScaleX = this.transform.lossyScale.x * 0.5f;
			Vector3 leftPoint = this.transform.position + halfScaleX * (Vector3.left * Mathf.Cos(this.transform.eulerAngles.z * Mathf.Deg2Rad) + Vector3.up * Mathf.Sin(-this.transform.eulerAngles.z * Mathf.Deg2Rad));
			return leftPoint;
		}

		/// <summary>
		/// Returns the right anchor point of the cable
		/// </summary>
		/// <returns></returns>
		public Vector3 GetRightPoint()
		{
			float halfScaleX = this.transform.lossyScale.x * 0.5f;
			Vector3 rightPoint = this.transform.position + halfScaleX * (Vector3.right * Mathf.Cos(this.transform.eulerAngles.z * Mathf.Deg2Rad) + Vector3.up * Mathf.Sin(this.transform.eulerAngles.z * Mathf.Deg2Rad));
			return rightPoint;
		}
		...
		protected virtual void ApplyBillboard(Camera targetCamera)
		{
